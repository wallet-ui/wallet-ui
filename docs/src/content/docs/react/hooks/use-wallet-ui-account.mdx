...
<Callout>
    For most common use cases, consider using the main [`useWalletUi`](/react/hooks/use-wallet-ui/) hook. It offers a
    simpler API with `connect()` and `disconnect()` functions instead of a raw state dispatcher.
</Callout>
