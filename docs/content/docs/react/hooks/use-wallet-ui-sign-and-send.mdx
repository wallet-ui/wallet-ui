...
- **`ix`**: `IInstruction | IInstruction[]` - A single instruction or an array of instructions to include in the transaction. These instructions should be created using a library like `gill`.
...
import { createTransferInstruction, getBase58Encoder } from 'gill';
