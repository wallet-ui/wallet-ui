...
<Callout>
    For simply accessing the currently selected cluster, consider using the main
    [`useWalletUi`](/react/hooks/use-wallet-ui/) hook. This hook is more focused on managing the cluster state, such as
    switching between different clusters.
</Callout>
